	...
	<form id="openai-form" action="" method="GET" enctype="multipart/form-data" class="mt-24">		
		...
		<div class="row justify-content-md-center">	
			<div class="col-sm-12 text-center">
				...
				<h6 class="mb-0 fs-12 text-muted">{{ isset($chat)?$chat->description:'' }}</h6>
				...
				<div class="chat-message-container" id="chat-system">
					<div class="card-header">
						<div class="w-100 pt-2 pb-2">
							<div class="d-flex">
								<div class="overflow-hidden mr-4"><img alt="Avatar" class="chat-avatar" src="{{ isset($chat) ? URL::asset($chat->logo) : '' }}"></div>
								<div class="widget-user-name"><span class="font-weight-bold">{{ isset($chat) ? __($chat->name) : '' }}</span><br><span class="text-muted">{{ isset($chat) ?__($chat->sub_name) : '' }}</span></div>
							</div>
						</div>
							...
							<div class="col-md-12 col-sm-12" >									
								<div id="chat-container">
									{{-- <div class="msg left-msg">
										<div class="message-img" style="background-image: url({{ isset($chat) ?$chat->logo:'' }})"></div>
										<div class="message-bubble">					
											<div class="msg-text">{{ isset($chat) ?__($chat->description):'' }}</div>
										</div>
									</div> --}}
									<div id="image-drop-box">
										<div class="image-drop-area text-center">
											<div class="msg left-msg text-center mt-9">
												<div class="message-img" style="background-image: url({{ isset($chat) ?$chat->logo:'' }})"></div>
												<div class="message-bubble">					
													<div class="msg-text">{{ __('Hey there, I can help you with analyzing any image that you upload') }}</div>
												</div>
											</div>
											...
											<p class="text-dark fw-bold mb-2 mt-3">
												{{ __('Drop your images to analyze here or') }}
												<a href="javascript:void(0);" class="text-primary">{{ __('Browse') }}</a>
											</p>
											<p class="mb-0 file-name text-muted">
												<small>{{ __('Allowed image formats') }}: .png .jpeg .webp</small>
											</p>
											<p class="mb-0 file-name text-muted">
												<small>{{ __('Max image size') }}: 20MB</small>
											</p>
											<div>
...
<script type="text/javascript">
	...
	const bot_avatar = "{{ isset($chat) ? $chat->logo : '' }}";
	const user_avatar = "{{ URL::asset(auth()->user()->profile_photo_path) }}";	
	const mic = document.querySelector('#mic-button');
	let eventSource = null;
	let isTranscribing = false;
	let chat_code = "{{ isset($chat) ? $chat->chat_code : '' }}";	
	let active_id;
	let default_message;
	let uploaded_image = '';
