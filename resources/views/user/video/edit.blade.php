...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('Edit Video') }}</h4>
			<ol class="breadcrumb mb-2">
				...
				<li class="breadcrumb-item active" aria-current="page"><a href="{{ route('user.videos.edit',$video->id) }}">{{ __('Edit Video') }}</a></li>
		...
		<div class="col-xl-9 col-lg-8 col-sm-12">
			<div class="card border-0">
				<div class="card-header">
					<h3 class="card-title">{{ __('Edit Video') }}</h3>
				</div>
				<div class="card-body pb-0">
					<form method="POST" action="{{ route('user.videos.update',$video->id) }}" enctype="multipart/form-data">
						@csrf
						...
						<div class="row">
							<div class="col-sm-6 col-md-6">
								<div class="input-box">
									<div class="form-group">
										...
										<input type="text" class="form-control @error('title') is-danger @enderror" name="title" value="{{ !empty($video->title) ? $video->title  : '' }}" required>
										@error('title')
                                ...
                                <div class="input-box">
                                    <div class="form-group">
										...
										<select id="section" class="form-control @error('section') is-danger @enderror" name="section" required>
                                            ...
                                            <option {{ ($video->section == 0 && old('section') == 0) ? 'selected' : '' }} value="0">Video</option>
                                            <option {{ ($video->section == 1 && old('section') == 14) ? 'selected' : '' }} value="1">Download</option>
                                        </select>
                                        @error('section')
								...
								<div class="input-box">
									<div class="form-group">
                                        ...
                                        <select class="form-control @error('category') is-danger @enderror" name="category" required>
                                            ...
                                            <option value="{{ $cate['id'] }}"  {{ $video->category == $cate['id'] ? 'selected' : '' }}>{{ $cate['name'] }}</option>
                                            @endforeach
                                        </select>
                                        @error('category')
                            ...
                            <div class="col-sm-6 col-md-6" id="embadded_sec" style="display:{{ ($video->section=='0') ? 'block' : 'none' }};">
								<div class="input-box">
									<div class="form-group">
										<label class="form-label fs-12">{{ __('Video URL') }} <span class="text-muted">({{ __('Required') }})</span></label>
										<input type="url" class="form-control @error('embadded_url') is-danger @enderror" name="embadded_url" id="embadded_url" value="{{ !empty($video->embadded_url) ? $video->embadded_url  : '' }}" {{ ($video->section=='0') ? 'required' : '' }}>
                                        @error('embadded_url')
                            ...
                            <div class="col-sm-6 col-md-6" id="download_sec" style="display:{{ ($video->section=='1' && old('section') != 1) ? 'block' : 'none' }};">
								<div class="input-box">
									<div class="form-group">
										<label class="form-label fs-12">{{ __('Download File URL') }} <span class="text-muted">({{ __('Required') }})</span></label>
										<input type="url" class="form-control @error('file_link') is-danger @enderror" name="file_link" id="file_link" value="{{ !empty($video->file_link) ? $video->file_link  : '' }}" {{ ($video->section=='1') ? 'required' : '' }}>
                                        @error('file_link')
